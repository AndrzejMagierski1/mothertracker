...
cSampleImporter* SI = &sampleImporter;
...
void cSampleImporter::start(uint32_t options)
{
	...
	for(uint8_t i = 0; i<4; i++)
	{
		...
		if(SI->topLabel[0] == nullptr) SI->topLabel[0] = display.createControl<cLabel>(&prop1);
	}
...
{
	//lista
	display.setControlHide(SI->fileListControl);

	// top label listy
	//display.setControlText(SI->topLabel[0], "");
	display.setControlHide(SI->topLabel[0]);
	...
	for(uint8_t i = 0; i<8; i++)
	{
		display.refreshControl(SI->bottomLabel[i]);
	}
...
void cSampleImporter::listOnlyFolderNames(const char* folder)
{
