 ...
 * Copyright (c) 2014 by Matthijs Kooijman <matthijs@stdin.nl> (SPI2Settings AVR)
...
public:
	...
	inline static void setClockDivider(uint8_t clockDiv) {
		if (clockDiv == SPI_CLOCK_DIV2) {
			setClockDivider_noInline(SPI2Settings(12000000, MSBFIRST, SPI_MODE0).ctar);
		} else if (clockDiv == SPI_CLOCK_DIV4) {
			setClockDivider_noInline(SPI2Settings(4000000, MSBFIRST, SPI_MODE0).ctar);
		} else if (clockDiv == SPI_CLOCK_DIV8) {
			setClockDivider_noInline(SPI2Settings(2000000, MSBFIRST, SPI_MODE0).ctar);
		} else if (clockDiv == SPI_CLOCK_DIV16) {
			setClockDivider_noInline(SPI2Settings(1000000, MSBFIRST, SPI_MODE0).ctar);
		} else if (clockDiv == SPI_CLOCK_DIV32) {
			setClockDivider_noInline(SPI2Settings(500000, MSBFIRST, SPI_MODE0).ctar);
		} else if (clockDiv == SPI_CLOCK_DIV64) {
			setClockDivider_noInline(SPI2Settings(250000, MSBFIRST, SPI_MODE0).ctar);
		} else { /* clockDiv == SPI_CLOCK_DIV128 */
			setClockDivider_noInline(SPI2Settings(125000, MSBFIRST, SPI_MODE0).ctar);
		}
	}
...
public:
	...
	inline static void beginTransaction(SPI2Settings settings) {
		...
		#endif
		SPI2_C1 = settings.c1;
		SPI2_BR = settings.br0;
	}

	// Write to the SPI bus (MOSI pin) and also receive (MISO pin)
	inline static uint8_t transfer(uint8_t data) {
		SPI2_DL = data;
		while (!(SPI2_S & SPI_S_SPRF)) ; // wait
		return SPI2_DL;
	}
	inline static uint16_t transfer16(uint16_t data) {
		SPI2_C2 = SPI_C2_SPIMODE;
		SPI2_S;
		SPI2_DL = data;
		SPI2_DH = data >> 8;
		while (!(SPI2_S & SPI_S_SPRF)) ; // wait
		uint16_t r = SPI2_DL | (SPI2_DH << 8);
		SPI2_C2 = 0;
		SPI2_S;
		return r;
	}
	...
	inline static void setClockDivider(uint8_t clockDiv) {
		if (clockDiv == SPI_CLOCK_DIV2) {
			SPI2_BR = (SPI2Settings(12000000, MSBFIRST, SPI_MODE0).br0);
		} else if (clockDiv == SPI_CLOCK_DIV4) {
			SPI2_BR = (SPI2Settings(4000000, MSBFIRST, SPI_MODE0).br0);
		} else if (clockDiv == SPI_CLOCK_DIV8) {
			SPI2_BR = (SPI2Settings(2000000, MSBFIRST, SPI_MODE0).br0);
		} else if (clockDiv == SPI_CLOCK_DIV16) {
			SPI2_BR = (SPI2Settings(1000000, MSBFIRST, SPI_MODE0).br0);
		} else if (clockDiv == SPI_CLOCK_DIV32) {
			SPI2_BR = (SPI2Settings(500000, MSBFIRST, SPI_MODE0).br0);
		} else if (clockDiv == SPI_CLOCK_DIV64) {
			SPI2_BR = (SPI2Settings(250000, MSBFIRST, SPI_MODE0).br0);
		} else { /* clockDiv == SPI_CLOCK_DIV128 */
			SPI2_BR = (SPI2Settings(125000, MSBFIRST, SPI_MODE0).br0);
		}
	}
...
public:
	...
	inline static void setClockDivider(uint8_t clockDiv) {
		if (clockDiv == SPI_CLOCK_DIV2) {
			SPI1_BR = (SPI2Settings(12000000, MSBFIRST, SPI_MODE0).br1);
		} else if (clockDiv == SPI_CLOCK_DIV4) {
			SPI1_BR = (SPI2Settings(4000000, MSBFIRST, SPI_MODE0).br1);
		} else if (clockDiv == SPI_CLOCK_DIV8) {
			SPI1_BR = (SPI2Settings(2000000, MSBFIRST, SPI_MODE0).br1);
		} else if (clockDiv == SPI_CLOCK_DIV16) {
			SPI1_BR = (SPI2Settings(1000000, MSBFIRST, SPI_MODE0).br1);
		} else if (clockDiv == SPI_CLOCK_DIV32) {
			SPI1_BR = (SPI2Settings(500000, MSBFIRST, SPI_MODE0).br1);
		} else if (clockDiv == SPI_CLOCK_DIV64) {
			SPI1_BR = (SPI2Settings(250000, MSBFIRST, SPI_MODE0).br1);
		} else { /* clockDiv == SPI_CLOCK_DIV128 */
			SPI1_BR = (SPI2Settings(125000, MSBFIRST, SPI_MODE0).br1);
		}
	}
...
class SPI2Settings {
public:
	SPI2Settings(uint32_t clock, BitOrder bitOrder, uint8_t dataMode) {
		if (__builtin_constant_p(clock)) {
			init_AlwaysInline(clock, bitOrder, dataMode);
		} else {
			init_MightInline(clock, bitOrder, dataMode);
...
class SPI2Class {
  public:
	SPI2Class(Spi *_spi, uint32_t _id, void(*_initCb)(void));
	...
	void beginTransaction(uint8_t pin, SPI2Settings settings);
	void beginTransaction(SPI2Settings settings) {
