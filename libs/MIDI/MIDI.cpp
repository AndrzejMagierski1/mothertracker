...
void MIDI_Class::begin(const byte inChannel, const byte set_serial_no)
{
	serial_no = set_serial_no;
	// Initialise the Serial port
	// zmodyfikowalem - odwrocenie logiki TX na potrzeny seqa, SERIAL_8N1
	Serial5.begin(MIDI_BAUDRATE, SERIAL_8N1_TXINV);
	//default
	//USE_SERIAL_PORT.begin(MIDI_BAUDRATE,SERIAL_8N1);
                      ...
                      byte channel)
{
	...
	if (type <= PitchBend) {
		...
		if (mRunningStatus_TX != statusbyte) {
			...
			Serial5.write(mRunningStatus_TX);
			//USE_SERIAL_PORT.write(mRunningStatus_TX);
		}
#else
		// Don't care about running status, send the Control byte.
		if (serial_no == 1)	Serial5.write(statusbyte);
		if (serial_no == 2)	Serial2.write(statusbyte);
...
void MIDI_Class::sendTimeCodeQuarterFrame(byte data)
{
	...
	Serial5.write((byte)TimeCodeQuarterFrame);
...
int MIDI_Class::buffersize()
{
	buff_bytes_left=Serial5.available();

	return buff_bytes_left;
}
...
void MIDI_Class::sendSongPosition(unsigned int Beats)
{
	...
	Serial5.write((byte)SongPosition);
	Serial5.write(Beats & 0x7F);
	Serial5.write((Beats >> 7) & 0x7F);
...
void MIDI_Class::sendSongSelect(byte SongNumber)
{
	...
	Serial5.write((byte)SongSelect);
	Serial5.write(SongNumber & 0x7F);
...
bool MIDI_Class::parse(byte inChannel)
{
	...
	bytes_available = Serial5.available();
