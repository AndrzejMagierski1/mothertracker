...
#else
uint8_t fourParamsOffset[4] =		{	9,  55, 88, 145,	};
uint8_t twoParamsOffsetFirst[4] =	{	9,  9,  3,   0,	};
uint8_t twoParamsOffsetSecond[4] =	{	0,  65, 52,  52,	};
#endif
...
/*
		...
		API_VERTEX2F(0, phy1);
		API_VERTEX2F(799, phy1);

		API_VERTEX2F(0, phy2);
		API_VERTEX2F(799, phy2);
*/
...
/*
		...
		//API_LINE_WIDTH(16);
		API_SCISSOR_XY(1, phy1-14);
		API_SCISSOR_SIZE(798, 14);
		API_CMD_GRADIENT(0, phy1-100, 0x565758, 0, phy1, 0x000000);
		...
		API_SCISSOR_SIZE(798, 14);
		API_CMD_GRADIENT(0, phy2, 0x111111, 0, phy2+14, 0x232323);
...
void cTracker::notes()
{
	...
	if(paramCount == 1) 		offset_x = rightOffset + ((tracksSpace/2)-18);
	...
	for(uint16_t i = 0; i < columnsCount; i++)
	{
		for(uint16_t j = 0; j < 15; j++)
		{
			...
			if(selectActive && mark>=0 && param_x > select1_x && param_x < select2_x && param_y > select1_y && param_y < select2_y) actualColor = colors[6];
...
void cTracker::instruments()
{
	...
	if(paramCount == 1) 		offset_x = rightOffset + ((tracksSpace/2)-12);
...
void cTracker::fxes1()
{
	...
	if(paramCount == 1) 		offset_x = rightOffset + ((tracksSpace/2)-24);
