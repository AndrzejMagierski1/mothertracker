...
void copy_step(uint8_t from_x, uint8_t from_y, uint8_t to_x, uint8_t to_y)
{
	...
	seq[player.ramBank].row[to_y].step[to_x] = seq[player.ramBank].row[from_y].step[from_x];
}
...
void initRow(uint8_t row, uint8_t bank)
{
	...
	for (uint8_t x = 1; x <= 32; x++)
	{
		...
		seq[bank].row[row].gateMode		= GATEMODE_NORMAL;
...
void randomize_row(uint8_t row)
{
	...
	int8_t random_from = constrain(seq[player.ramBank].row[row].rootNote - RANDOM_NOTE_DOWN,
	                               MIN_NOTE_TRACK,
	                               MAX_NOTE_TRACK);
	int8_t random_to = constrain(seq[player.ramBank].row[row].rootNote + RANDOM_NOTE_UP,
	                             MIN_NOTE_TRACK,
	                             MAX_NOTE_TRACK);
	...
	for (uint8_t col = 1; col <= 32; col++)
	{
		seq[player.ramBank].row[row].step[col].note = seq[player.ramBank].row[row].rootNote;
		seq[player.ramBank].row[row].step[col].chord = MIN_CHORD;

		if (oneRoll)
		{
			seq[player.ramBank].row[row].step[col].hitMode = random(0, dispersion) < 1;
			if (seq[player.ramBank].row[row].step[col].hitMode) seq[player.ramBank].row[row].step[col].isOn = 1;
			else 												clearStep(col, row);
		}
		else
		{
			if (random(0, 4) > 2)
			...
			if (seq[player.ramBank].row[row].step[col].hitMode > 1) oneRoll = 1;

		}


		if (seq[player.ramBank].row[row].step[col].hitMode > 1 && seq[player.ramBank].row[row].step[col].isOn)
		{
			seq[player.ramBank].row[row].step[col].rollCurve = random(ROLL_CURVE_MIN, ROLL_CURVE_MAX + 1);
		}
		if (seq[player.ramBank].row[row].step[col].hitMode > 0 && seq[player.ramBank].row[row].step[col].isOn)
		{

			// jeżeli nie rolka, spróbuj randomizować nudge
			if ((seq[player.ramBank].row[row].step[col].hitMode == 1) && seq[player.ramBank].row[row].step[col].isOn && (seq[player.ramBank].row[row].flags & MASK_RANDOM_NUDGE))
			{
				if (random(0, 2) < 1)
				{
					seq[player.ramBank].row[row].step[col].hitMode = HITMODE_OFFSET;
					seq[player.ramBank].row[row].step[col].offset = random(OFFSET_MIN, OFFSET_MAX);;
				}
				else
				{
			...
			{
				seq[player.ramBank].row[row].step[col].velocity = random(RANDOM_VELO_MIN, RANDOM_VELO_MAX);
			}
			else
			{
			...
			{
				seq[player.ramBank].row[row].step[col].modulation = random(RANDOM_MOD_MIN, RANDOM_MOD_MAX);
			}
			else
			{
			...
			if (randomInScale)
			{
				...
				{
					seq[player.ramBank].row[row].step[col].note = random(random_from, random_to);

					if (isInScale(seq[player.ramBank].row[row].step[col].note,
					              seq[player.ramBank].row[row].rootNote,
					              seq[player.ramBank].row[row].trackScale)) break;

					if (a == 15)seq[player.ramBank].row[row].step[col].note = seq[player.ramBank].row[row].rootNote;
				}
...
int8_t getNextRollNoteOffset(uint8_t row)
{
	...
	if (player.row[row].rollStep == 0) return 0;

	strStep & step = seq[player.ramBank].row[row].step[player.row[row].rollStep];

	if 		(step.rollNoteCurve == ROLL_CURVE_FLAT) 		return 0;
	else if (step.rollNoteCurve == ROLL_CURVE_INC)			return player.row[row].rollCounter - 1;
	else if (step.rollNoteCurve == ROLL_CURVE_DEC)			return -player.row[row].rollCounter + 1;

	else if (step.rollNoteCurve == ROLL_CURVE_INC_DEC)
	{
	...
	else if (step.rollNoteCurve == ROLL_CURVE_DEC_INC)
	{
		uint8_t total = (step.length1 + 1) * val2roll(step.hitMode);
	...
	else if (step.rollNoteCurve == ROLL_CURVE_RANDOM)
	{
		retVal = random(-6, 6);
	}
...
uint8_t getLongRollVelo(uint8_t rollCurve, float progress)
{
	...
	else if (rollCurve == ROLL_CURVE_INC)
	{
		retVal = (progress / 100) * 127;
	}

	else if (rollCurve == ROLL_CURVE_DEC)
	{
...
void init_player_timer(void) // MT::refreshTimer
{
	...
	if (config.mode == MODE_MIDICLOCK_INTERNAL) 			temp_Tempo = seq[player.ramBank].tempo;
	else if (config.mode == MODE_MIDICLOCK_INTERNAL_LOCK) 	temp_Tempo = config.tempoLock;
	else 							 						temp_Tempo = player.externalTempo;
...
void handle_uStep_timer(void)
{
	...
	*/
	...
	// noInterrupts();
	if ((config.mode == MODE_MIDICLOCK_INTERNAL) || (config.mode == MODE_MIDICLOCK_INTERNAL_LOCK))
	{
		// handle_uStep(0);
		// play_uStepEmulate(0);
		if (isPlay() || isREC())
...
void handle_uStep12(uint8_t step)
{
	if (isPlay())
	{
		...
		if ((step == 1))// to znaczy że wywoładnie funkcji przyszło z midi clocka
		{
			...
			{
				for (uint16_t tempTick = tickBefore; tempTick <= tickAfter; tempTick++)
				{
					if (tempTick > 6912)
					{
						tempTick = 1;
					...
					for (uint8_t a = 1; a <= 8; a++)
					{
						int8_t tTempoOption = constrain(seq[player.ramBank].row[a].tempoDiv, MIN_TEMPO_DIV, MAX_TEMPO_DIV);
						uint8_t tDiv      = getTempoDiv(tTempoOption);
		...
		else // wywoładnie z timera wewnętrznego
		{
			if (!player.isREC)
			{
				for (uint8_t a = 1; a <= 8; a++)
				{
					int8_t tTempoDiv = constrain(seq[player.ramBank].row[a].tempoDiv, MIN_TEMPO_DIV, MAX_TEMPO_DIV);
		...
		if ((timerTick % 12 == 1) || step)
		{
			if (player.uStep == 1)
			{
				...
				{
					float tempSwing;
					if (config.mode == MODE_MIDICLOCK_INTERNAL) 			tempSwing = seq[player.ramBank].swing;
					else if (config.mode == MODE_MIDICLOCK_INTERNAL_LOCK) 	tempSwing = config.swingLock;
					else tempSwing = 50.0;


					if ((player.swingToogle) ) player.swing_offset = 0 + tempSwing;
					else player.swing_offset = 100 - tempSwing;
	...
	if ((timerTick % 12 == 1) || step) rec_metronome();
...
float get_swing(void)
{
	...
	if (config.mode == MODE_MIDICLOCK_INTERNAL)				return seq[player.ramBank].swing;
	else if (config.mode == MODE_MIDICLOCK_INTERNAL_LOCK) 	return config.swingLock;
	else 													return 0;
}
